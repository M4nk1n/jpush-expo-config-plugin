...
// 配置 iOS AppDelegate
const setAppDelegate = config => withAppDelegate(config, config => {
...
const setAndroidManifest = config => withAndroidManifest(config, config => {
  if (AndroidConfig.Manifest.findMetaDataItem(
  ...
  if (config.modResults.manifest.application[0].activity.findIndex(
